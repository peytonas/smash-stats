...
class OpponentName extends Component {
  ...
  render() {
    ...
    return (
      <p className={this.props.opponent.name === name ? "border-danger name-size" : null || "name-size"} style={{ order: characterNumber }} onClick={this.selectCharacter.bind(this, this.props.character._id)}><b className="ml-1">{name}</b></p>
    )
  }
}
